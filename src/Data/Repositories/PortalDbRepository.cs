...
{
    public class PortalDbRepository<T> : IRepository<T> where T : class
    {
        protected PortalDbContext Context { get; set; }

        public PortalDbRepository(PortalDbContext context)
        {
            Context = context;
        }
        ...
        {
            return new PortalDbRepository<TK>(Context);
        }
    }
